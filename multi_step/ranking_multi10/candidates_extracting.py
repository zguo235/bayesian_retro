...
test_2steps = pd.read_pickle('data/preprocessed_liu_dataset/test_2steps_addition.pickle')
...
for i, result in enumerate(results):
    ...
    print('Reaction {}:'.format(reaction_num), 'test {},'.format(i), 'number of candidates: {}'.format(len(df)), flush=True)
    if len(df) > 0:
...
print('Finished Reaction {}'.format(reaction_num))
